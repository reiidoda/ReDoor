...
async fn main() -> Result<()> {
    ...
    let directory_url = crate::config::default_directory_url();
            ...
            "3" => {
                ...
                let bundle_json = if input.starts_with("{") {
                    ...
                    if let Some(directory_client) = directory_client {
                        ...
                        match result {
                            Ok(pk) => {
                                let bytes = engine.runtime.block_on(
                                    crate::orchestrator::fetch_prekey_bundle(
                                        &directory_client,
                                        &pk,
                                        3,
                    ...
                    if let Some(my_id) = &guard.identity {
                        ...
                        match x3dh::initiate_handshake(my_id, &peer_bundle) {
                            Ok((shared_secret, initial_msg)) => {
                                ...
                                let peer_spk = crate::crypto::x25519::PublicKey::from(spk_bytes);

                                let session = crate::engine::SessionEntry {
                                    wrapped_state: None,

                                    inner: Some(
                                        crate::ratchet::double_ratchet::RatchetSession::new(
                                            shared_secret,
                                            Some(peer_spk),
                                        ),
                                    ),
                                ...
                                if let Some(client) = relay_client {
                                    ...
                                    let mailbox_id = hex::encode(crate::crypto::blake3::hash(
                                        peer_id.as_bytes(),
                                    ));
                                    ...
                                    let res = engine.runtime.block_on(async {
                                        if onion_routing {
                                            crate::orchestrator::send_onion_blob_with_retry(
                                                &onion_router,
                                                &client,
                                                &hex::encode(crate::crypto::blake3::hash(&blob)),
                                                &peer_id,
                                                &blob,
                                                3,
            ...
            "8" => {
                ...
                if let (Some(directory_client), Some(identity)) = (directory_client, identity) {
                    ...
                    let upload_result =
                        engine
                            .runtime
                            .block_on(crate::orchestrator::upload_prekey_bundle(
                                &directory_client,
                                &public_key_hex,
                                &bundle_bytes,
                                3600,
                    ...
                    let signature = hex::encode(identity.sign(
                        &crate::network::directory::publish_signing_message(
                            username,
                            &public_key_hex,
                            seq,
                            expires_at,
