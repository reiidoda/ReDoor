...
- iOS production runtime path still has stub cleanup/wiring work tracked in `#212`.
- Relay connection telemetry exposure in FFI is tracked in `#213`.
- Deterministic integration-test promotion in CI is tracked in `#214`.
- Roadmap/docs status reconciliation is tracked in `#215`.
